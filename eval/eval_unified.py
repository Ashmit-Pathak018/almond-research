...
from core.almond import Almond, AlmondConfig
from core.memory_block import MemoryTag, MemoryTier
from core.memory_controller_v2 import EvictionPolicy
...
def resolve_dataset_path(dataset_path: str | Path) -> Path:
    """Resolve a dataset path from the repo root when needed."""
    path = Path(dataset_path)
    if path.is_absolute():
        return path

    candidates = [Path.cwd() / path, ROOT_DIR / path]
...
from eval.judge_v2 import judge as _judge_v2, JudgeResult
...
def apply_ablation(almond: Almond, ablation: str):
    ...
    import copy
    try:
        from core.memory_pipeline_v2.ranking_engine import _WEIGHT_PROFILES
    except ImportError:
